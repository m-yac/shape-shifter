 ...
 * Centered over everything (the library included), with a solid backlight
...
export class HelpDialog {
  ...
  // Any click closes the dialog, except one on the Help button itself: that is
  // left to the button's own toggle, so it doesn't dismiss-then-reopen.
  ...
  show(variant: HelpVariant = "main"): void {
    ...
    fadeIn(popup.el, 0.3);
    this.popup = popup;
    // Safe to attach synchronously: this runs from the Help button's `click`, which
    // fires after its `pointerup`, so the opening gesture never reaches this listener.
